    ...
    public class UserEdits : ManagePageBase
    {
        ...
        private void Save()
        {
            ...
            if (itemInfoId > 0)
            {
                ...
                {
                    PromotionUserFactory.Insert(new PromotionUserInfo { PID = int.Parse(this.ddlagents.SelectedValue), Prices = 0.5M, RegId = itemInfoId, PromTime = DateTime.Now, PromStatus = 1 });
                }
                else
                {
