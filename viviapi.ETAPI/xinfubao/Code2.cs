...
{
    public class Code2 : ETAPIBase
    {
        private static int suppId = 10005;
        internal string notifyUrl
        {
            get
            {
                return this.SiteDomain + "/notify/xfb/Notify2.aspx";
            }
        }
        ...
        public void Notify()
        {
            ...
            if (signed.ToUpper().Equals(signData.ToUpper()))
            {
                ...
                if (list["orderStatus"].Equals("01"))
                {
                    ...
                    new OrderBank().DoBankComplete(Code2.suppId, list["prdOrdNo"], list["payId"], status, opstate, msg, Decimal.Parse(list["orderAmount"]) / 100m, new Decimal(0), true, false);
