...
wojilu.ui.valid = function() {
    ...
    arrRule['ddl'] = /^[1-9]\d{0,5}$/;
    arrRule['integer']= /^-?[1-9]\d*$/; //整数   
    arrRule['integer1']= /^[1-9]\d*$/; //正整数
    arrRule['integer2']= /^-[1-9]\d*$/; //负整数      
    arrRule['decmal1']= /^[1-9]\d*.\d*|0.\d*[1-9]\d*$/; //正浮点数
...
function setServerId(serverid) {
         ...
         if (groupid == '') {
             ...
             var allOptions = "," + $("#Group" + groupid).val().replace("，", ",");
             //allOptions = allOptions.replace(new RegExp("\n", "gm"), ",");
             //alert(groupid);
             //alert($('#Group' + groupid).val());
             if ($('#Group' + groupid).val().length == 0) {
                 $('#Group' + groupid).val($('#Group' + groupid).val() + serverid);
             }
             else {
                 if (allOptions.indexOf("," + serverid) < 0) {
                     $("#Group" + groupid).val($("#Group" + groupid).val() + "," + serverid);
                 }
             }
         }
